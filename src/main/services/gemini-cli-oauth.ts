...
export interface GeminiCliOAuthContext {
...
function findFile(dir: string, name: string, depth: number): string | null {
  ...
  try {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isFile() && entry.name === name) return next
...
async function installViaNpm(onProgress?: (msg: string) => void): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    ...
    child.stderr?.on('data', () => { /* suppress npm noise */ })
  })
}
...
async function installViaDirectDownload(onProgress?: (msg: string) => void): Promise<boolean> {
  try {
    ...
    const metaRes = await net.fetch('https://registry.npmjs.org/@google/gemini-cli-core/latest')
    if (!metaRes.ok) {
    ...
    const tarRes = await net.fetch(tarballUrl)
    if (!tarRes.ok) {
    ...
    try {
      execFileSync('tar', ['xzf', tmpFile, '-C', targetDir, '--strip-components=1'], { timeout: 30_000 })
    } finally {
...
async function ensureOAuthClientConfig(
  ...
  if (installed) {
    ...
    if (installedExtracted) return installedExtracted
  }
...
async function getUserEmail(accessToken: string): Promise<string | undefined> {
  try {
    const response = await net.fetch(USERINFO_URL, {
...
async function pollOperation(
  ...
  for (let attempt = 0; attempt < 24; attempt++) {
    ...
    const response = await net.fetch(`${CODE_ASSIST_ENDPOINT}/v1internal/${operationName}`, { headers })
...
async function discoverProject(accessToken: string): Promise<string> {
  ...
  const response = await net.fetch(`${CODE_ASSIST_ENDPOINT}/v1internal:loadCodeAssist`, {
    method: 'POST',
    headers,
  ...
  const onboardResponse = await net.fetch(`${CODE_ASSIST_ENDPOINT}/v1internal:onboardUser`, {
    method: 'POST',
    headers,
  ...
  if (!onboardResponse.ok) {
    ...
    throw new Error(`Google project provisioning failed (${onboardResponse.status}): ${respText}`)
  }
