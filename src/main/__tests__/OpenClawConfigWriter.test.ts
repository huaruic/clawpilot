...
const FAKE_ROUTING = path.join(FAKE_STATE, 'routing.json')
...
vi.mock('../services/RuntimeLocator', () => ({
  ...
  getDefaultOpenClawWorkspaceRoot: vi.fn(() => path.join('/fake', 'workspace')),
}))
...
describe('OpenClawConfigWriter', () => {
  ...
  describe('writeRoutingConfig', () => {
    it('writes to routing.json', async () => {
      ...
      const written = writtenFiles.get(FAKE_ROUTING)
      expect(written).toBeDefined()
      const parsed = JSON5.parse(written!)
